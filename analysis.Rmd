...
```{r}
#library(MASS)
#How do I run a time series using gam? From what I've seen I add interaction terms. I need to do time series with spatial considerations. How is this done?
#Data does not allow for interactions... to many 

swk.fit = gam(smr ~ s(prec_mean) +
...
```{r}
swk.int = gam(smr ~ district +
                year +
                s(prec_mean) +
                s(broad_evg_clop_prop) +
                te(prec_mean, by = district) +
                te(prec_mean, by = year) +
                te(broad_evg_clop_prop, by = district)
              ,
              data = swk)
