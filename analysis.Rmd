...
```{r}
sbh.x = model.matrix(smr~., sbh_sub)[,-1]
sbh.y = sbh_sub$smr
...
```{r}
gam1 = bam(prec_mean ~ district + s(as.integer(year), by = district) + s(as.integer(year)),
...
gam_sbh = bam(smr ~ 
                ...
                year +
                district, 
              data = sbh, method = 'REML', family = poisson(link = log)) 
